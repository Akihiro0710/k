...
RUN apt update && apt upgrade --yes && apt install --yes                 \
         git debhelper maven openjdk-8-jdk cmake libboost-test-dev       \
         libyaml-cpp-dev libjemalloc-dev flex bison clang-6.0 zlib1g-dev \
         libgmp-dev libmpfr-dev gcc z3 libz3-dev opam pkg-config curl cvc4
